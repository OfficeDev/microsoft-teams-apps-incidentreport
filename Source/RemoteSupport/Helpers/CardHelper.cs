...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        public static string ValidateAdditionalTicketDetails(string additionalDetails, TimeSpan timeSpan)
        {
            ...
            if (details != null)
            {
                ...
                {
                    if (DateTime.TryParse(item.Value, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime inputDateTime))
                    {
                        keyValuePair.Add(item.Key, TicketHelper.ConvertToDateTimeoffset(inputDateTime, timeSpan).ToString(CultureInfo.InvariantCulture));
                    }
                    else
                    {
